...
:githubsrc: https://github.com/couchbaselabs/userprofile-couchbase-mobile-android/tree/sync/content/modules/userprofile-sync-android/examples/src
...
* Setup the Couchbase Sync Gateway to sync content between multiple Couchbase Lite enabled clients.
...
====
You can learn more about Sync Gateway here in the {sync-gateway-docs--xref}
====
****
...
** The fundamentals of using Couchbase Lite, in the {standalone--xref}

** The {query--xref}, which introduces Query basics, together with use of a prebuilt database
...
We will be working with a simple "User Profile" app which we introduced in the {standalone--xref} and extended in the {query--xref}.
...
If have followed the {query--xref}, you can skip this section and proceed to the <<Backend Installation>>. section.
...
* Open the `sync-gateway-config-userprofile-demo-2-x.json*` file using any text editor of your choice.
This configuration file is located in the app bundle at `/path/to/UserProfileDemo/modules/userprofile/examples/src`.
...
We use the
{sync-function-api-require-user--xref}
...
====
You can learn more about the Sync Function in the
{sync-function--xref}
...
{channels--xref} are a mechanism to "tag" documents and is typically used to segregate documents based on the contents of the document.
Combined with the
{sync-function-api-access--xref}
and
{sync-function-api-require-access--xref}
API, it can be used to enforce <<Access Control>>.
...
You can enforce access control to channels using the
{sync-function-api-access--xref}
API.
This will ensure that only users with access to a specific channel will be able to retrieve documents in the channel.
...
<3> The `SelectResult.all()` method is used to query all the properties of a document.
...
So we retrieve the Dictionary at key _"userprofile"_.
