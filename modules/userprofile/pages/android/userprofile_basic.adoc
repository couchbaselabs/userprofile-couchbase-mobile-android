...
This tutorial will walk you through a very basic example of how you can use *Couchbase Lite 2.x in standalone mode* within your link:https://www.android.com[*Android*]  app.
...
====
You can learn more about Couchbase Mobile https://docs.couchbase.com/home/mobile.html[here]
====

== Prerequisites

This tutorial assumes familiarity with building Android apps using https://www.java.com[Java].

* https://developer.android.com/studio[Android Studio 3.4 or above]
...
- Clone the *_standalone_* branch of the `User Profile Demo` repository from GitHub. Assuming that you have link:https://git-scm.com/downloads[Git] installed you can use the following command to clone the solution:
+
[source,bash]
----
...
- Download the  `User Profile Demo` solution directly from link:https://github.com/couchbaselabs/userprofile-couchbase-mobile-android/archive/standalone.zip[here].
...
```
<1> Special https://docs.couchbase.com/couchbase-lite/current/android/learn/java-android-blob.html[`Blob`] data type that is associated with the profile image.
...
Before you can start using Couchbase Lite on Android, you would have to initialize it. Starting with v2.6, Couchbase Lite needs to be initialized with appropriate Android Application Context. Prior to 2.6, you would specify this in the `DatabaseConfiguration`

* Open the link:{githubsrc}/app/src/main/java/com/couchbase/userprofile/util/DatabaseManager.java[*DatabaseManager.java*] file and locate the `initCouchbaseLite` method.
+
[source,java]
----
include::{examplesdir}/src/app/src/main/java/com/couchbase/userprofile/util/DatabaseManager.java[tags=initCouchbaseLite,indent=0]
----
...
* Open the link:{githubsrc}/app/src/main/java/com/couchbase/userprofile/util/DatabaseManager.java[*DatabaseManager.java*] file and locate the `openOrCreateDatabaseForUser` method.
+
[source,java]
----
include::{examplesdir}/src/app/src/main/java/com/couchbase/userprofile/util/DatabaseManager.java[tags=openOrCreateDatabase,indent=0]
----

* We create an instance of the `DatabaseConfiguration` within link:{githubsrc}/app/src/main/java/com/couchbase/userprofile/util/DatabaseManager.java[*DatabaseManager.java*] via an `abstract` requirement from the parent class, `BaseRepository`. This is an optional step. In our case, we would like to override the default path of the database. Every user has their own instance of the `Database` that is located in a folder corresponding to the user. Please note that the default path is platform specific.
+
[source,java]
----
include::{examplesdir}/src/app/src/main/java/com/couchbase/userprofile/util/DatabaseManager.java[tags=databaseConfiguration,indent=0]
----
...
[source,java]
----
include::{examplesdir}/src/app/src/main/java/com/couchbase/userprofile/util/DatabaseManager.java[tags=createDatabase,indent=0]
----
...
* Open the link:{githubsrc}/app/src/main/java/com/couchbase/userprofile/util/DatabaseManager.java[*DatabaseManager.java*] file and locate the `registerForDatabaseChanges()` function.
[source,java]
+
----
include::{examplesdir}/src/app/src/main/java/com/couchbase/userprofile/util/DatabaseManager.java[tags=registerForDatabaseChanges,indent=0]
----
...
[source,java]
----
include::{examplesdir}/src/app/src/main/java/com/couchbase/userprofile/util/DatabaseManager.java[tags=addDatabaseChangelistener,indent=0]
----
...
* Open the link:{githubsrc}/app/src/main/java/com/couchbase/userprofile/util/DatabaseManager.java[*DatabaseManager.java*] file and locate the `closeDatabaseForCurrentUser()` function.
+
[source,java]
----
include::{examplesdir}/src/app/src/main/java/com/couchbase/userprofile/util/DatabaseManager.java[tags=closeDatabaseForUser,indent=0]
----
...
[source,java]
----
include::{examplesdir}/src/app/src/main/java/com/couchbase/userprofile/util/DatabaseManager.java[tags=closeDatabase,indent=0]
----

=== De-registering from Database Changes

* Open the link:{githubsrc}/app/src/main/java/com/couchbase/userprofile/util/DatabaseManager.java[*DatabaseManager.java*] file and locate the `deregisterForDatabaseChanges()` function.
+
[source,java]
----
include::{examplesdir}/src/app/src/main/java/com/couchbase/userprofile/util/DatabaseManager.java[tags=deregisterForDatabaseChanges,indent=0]
----
...
[source,java]
----
include::{examplesdir}/src/app/src/main/java/com/couchbase/userprofile/util/DatabaseManager.java[tags=removedbchangelistener,indent=0]
----
...
* Open the link:{githubsrc}/app/src/main/java/com/couchbase/userprofile/util/DatabaseManager.java[*DatabaseManager.java*] file and locate the `getCurrentUserDocId` method. This document Id is constructed by prefixing the term "user::" to the email Id of the user.
+
[source,java]
----
include::{examplesdir}/src/app/src/main/java/com/couchbase/userprofile/util/DatabaseManager.java[tags=userProfileDocId,indent=0]
----

* Next, in the link:{githubsrc}/app/src/main/java/com/couchbase/userprofile/profile/UserProfilePresenter.java[*UserProfilePresenter.java*] file, locate the `fetchProfile` method.
+
[source,java]
----
include::{examplesdir}/src/app/src/main/java/com/couchbase/userprofile/profile/UserProfilePresenter.java[tags=fetchProfile,indent=0]
----
+
**Note**: The `fetchProfile` method is required by link:{githubsrc}/app/src/main/java/com/couchbase/userprofile/profile/UserProfileContract.java[*UserProfileContract.java*], playing a key role in the MVP architecture previously mentioned.

* We try to fetch the `Document` with specified `userProfileDocId` from the database.
+
[source,java]
----
include::{examplesdir}/src/app/src/main/java/com/couchbase/userprofile/profile/UserProfilePresenter.java[tags=docfetch,indent=0]
----
...
<5> Using the newly constructed <<UserProfile>> update the UI via the `showProfile` method (required by the interface link:{githubsrc}/app/src/main/java/com/couchbase/userprofile/profile/UserProfileContract.java[**UserProfileContract**] which is implemented by link:{githubsrc}/app/src/main/java/com/couchbase/userprofile/profile/UserProfileActivity.java[**UserProfileActivity**]).

=== Creating / Updating a Document

A <<The "User Profile" Document>> is created for the user when the user taps the "Save" button on the "Profile Screen".
The method below applies whether you are creating a document or updating an existing version.

* Open the link:{githubsrc}/app/src/main/java/com/couchbase/userprofile/profile/UserProfilePresenter.java[*UserProfilePresenter.java*] file and locate the `saveProfile` method.
+
[source,java]
----
include::{examplesdir}/src/app/src/main/java/com/couchbase/userprofile/profile/UserProfilePresenter.java[tags=saveProfile,indent=0]
----
...
[source,java]
----
include::{examplesdir}/src/app/src/main/java/com/couchbase/userprofile/profile/UserProfilePresenter.java[tags=docset,indent=0]
----
+
* Save the document.
+
[source,java]
----
include::{examplesdir}/src/app/src/main/java/com/couchbase/userprofile/profile/UserProfilePresenter.java[tags=docsave,indent=0]
----
...
+
image::{imagesdir}/android/doc_create.png[User Profile Document Creation]
+
* Now tap on the "Upload Photo" button and select an image from the Photo Album.
+
image::{imagesdir}/android/image_selection.gif[100,400]
+
* Tap "Save"
* Confirm that you see toast message "Succesfully Updated Profile". The `Document` will be updated this time.
* Tap "Log Out" and log out of the app
* Log back into the app with the same user email Id and password that you used earlier. In my example, I used _"+demo@example.com+"_ and _"password"_. (Logging in with those credentials again)
* Confirm that you see the profile screen  with the _name_ and _image_ values that you set earlier.
+
image::{imagesdir}/android/log_off_on.gif[200,400]

== Learn More

Congratulations on completing this tutorial!

This tutorial walked you through a very basic example of how to get up and running with Couchbase Lite as a local-only, standalone embedded data store in your Android app. If you want to learn more about Couchbase Mobile 2.x, check out the following links.
...
* link:https://docs.couchbase.com/couchbase-lite/current/introduction.html[Couchbase Lite Reference Guide]
