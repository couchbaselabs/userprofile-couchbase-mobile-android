...
:page-aliases: tutorials:userprofile-couchbase-mobile-android:standalone/userprofile/userprofile_basic
...
----
<1> Create an instance the <<UserRecord>> via `Map<String,Object>`.
...
<5> Using the newly constructed <<UserRecord>> update the UI via the `showProfile` method (required by the interface link:{githubsrc}/app/src/main/java/com/couchbase/userprofile/profile/UserProfileContract.java[**UserProfileContract**] which is implemented by link:{githubsrc}/app/src/main/java/com/couchbase/userprofile/profile/UserProfileActivity.java[**UserProfileActivity**]).
...
* Open the link:{githubsrc}/app/src/main/java/com/couchbase/userprofile/profile/UserProfilePresenter.java[*UserPresenter.java*] file and locate the `saveProfile` method.
+ 
[source,java]
----
...
* Confirm that you see a toast message "Succesfully Updated Profile". The first time, you update the profile screen, the `Document` will be created. 
+
